    ...
    def forward(self, image, label, *args, **kwargs):
        ...
        for name, operator in self.student.named_modules():
            ...
            if hasattr(operator, 'routing_weights_cache'):
                ...
                logits_route = get_module_dict(self.projector, new_name)(operator.routing_weights_cache) #将logit交给对应位置的projector处理，输出1*num_classes tensor
                #temp_c = logits_route.shape
                #temp_d = logits_student.shape
...
def routing_loss(routing_weights, weights_teacher, temperature=1.): #weights_teacher refer to weights calculate by teacher logit
    ...
    #moe_loss = temperature ** 2 * (1-pearson_correlation(routing_weights, weights_teacher)) #use soft loss to further soften the constrain
    return moe_loss
